...
function logWarning(context, error) {
    console.warn(`[Heron.MudCalendar] ${context}`, error);
}
    ...
    try {
        ...
        element.querySelectorAll(".mud-cal-month-row-holder").forEach(function(container) {
            try {
                ...
                logWarning("Unable to position items for a month row.", error);
            }
        });
    }
    catch (error) {
        logWarning("Unable to position month items.", error);
    }
}
        ...
        if (_obj) {
            ...
            div.addEventListener("click", async () => {
                try {
                    ...
                    logWarning("Unable to handle overflow message click.", error);
                }
            });
        }
    }
    catch (error) {
        logWarning("Unable to calculate overflow items.", error);
    }
}
